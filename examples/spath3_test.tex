...
;
\draw[spath/save global=a, line width=5pt, orange] (2,1) -- ++(1,1) -- +(1,-1) [show last coords];
\end{scope}
\path
(spath cs:a 1) node[circle,fill=blue] {}
...
\tikzset{spath/show=a}
\draw[spath/restore=a,show last coords] -- +(0,1);
\tikzset{spath/show=a}
\begin{scope}[scale=3]
\draw[spath/restore=a,show last coords] -- +(0,1);
\end{scope}
\draw[spath/restore=a] to[out=-90,in=-90] +(1,0);
\draw[spath/restore=a] to[out=-90,in=-90] cycle;
\end{tikzpicture}
\end{document}
